...
var DEFAULT_PREFERENCES = {
	...
	enableWebGL: false,
	rangeChunkSize: 65536, // bật chunk loading
	disableStream: false,
	disableRange: false,
	...
	PDFFindController.prototype = {
		...
		extractText: function PDFFindController_extractText() {
			...
			function extractPageText(pageIndex) {
				self.pdfPageSource.pages[pageIndex]
					.getTextContent()
...
var DocumentProperties = {
	...
	getProperties: function documentPropertiesGetProperties() {
		...
		PDFView.pdfDocument.getDownloadInfo().then(
			function (data) {
		...
		PDFView.pdfDocument.getMetadata().then(
			function (data) {
				var fields = [
					{
...
var PDFView = {
	...
	load: function pdfViewLoad(pdfDocument, scale) {
		...
		Promise.all([
			...
			function resolved() {
				...
				if (showPreviousViewOnLoad && store.get('exists', false)) {
					...
					var zoom =
						defaultZoomValue || store.get('zoom', PDFView.currentScale);
		...
		var destinationsPromise = (this.destinationsPromise =
	...
	renderHighestPriority: function pdfViewRenderHighestPriority(
		...
		var pageView = this.getHighestPriority(
			visiblePages,
			this.pages,
		...
		if (this.sidebarOpen) {
			...
			var thumbView = this.getHighestPriority(
				visibleThumbs,
				this.thumbnails,
	...
	getHighestPriority: function pdfViewGetHighestPriority(
		visible,
		views,
		scrolledDown
	) {
		...
		switch (view) {
			case 'thumbs':
				...
				if (wasAnotherViewVisible) {
					...
					scrollIntoView(
						document.getElementById('thumbnailContainer' + this.page)
					);
				}
				break;
...
var PageView = function pageView(
	...
	this.scrollIntoView = function pageViewScrollIntoView(dest) {
		...
		var pageWidth =
			(changeOrientation ? this.height : this.width) / this.scale / CSS_UNITS;
		var pageHeight =
			(changeOrientation ? this.width : this.height) / this.scale / CSS_UNITS;
		var scale = 0;
		switch (dest[1].name) {
			case 'XYZ':
				x = dest[2];
			...
			case 'FitR':
				...
				widthScale =
					...
					CSS_UNITS;
				heightScale =
					(PDFView.container.clientHeight - SCROLLBAR_PADDING) /
					height /
					CSS_UNITS;
...
//Add
function posMess(str) {
	parent.parent.postMessage(str, 'do something');
}
function webViewerInitialized() {
	...
	if (!PDFView.supportsFullscreen) {
		...
		//dtsang add
		if (document.getElementById('presentationMode_fullview')) {
			document
				.getElementById('presentationMode_fullview')
				.addEventListener('click', function () {
					//parent.parent.location.href = location.href.replace("smallviewer.html", "viewer.html") + "&url=" + encodeURI(parent.parent.location.href);
					var url =
						'PdfReadersmall;' +
						location.href.replace('smallviewer.html', 'viewer.html');
					posMess(url);
				});
		}
	} else {
	...
	if (document.getElementById('btnback')) {
		document.getElementById('btnback').addEventListener('click', function () {
			//parent.parent.location.href = location.href.split("&url=")[1];
			var len = location.href.split('&url=').length;
			var url = 'PdfReader;' + location.href.split('&url=')[len - 1];
			posMess(url);
		});
	}
	...
	if (cmd === 0) {
		...
		switch (evt.keyCode) {
			...
			case 35: // end
				if (
					PresentationMode.active ||
					(PDFView.pdfDocument && PDFView.page < PDFView.pdfDocument.numPages)
				) {
					PDFView.page = PDFView.pdfDocument.numPages;
					handled = true;
				}
